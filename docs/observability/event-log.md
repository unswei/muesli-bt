...
- `vla_submit`, `vla_poll`, `vla_cancel`, `vla_result`, `walking_target_dispatch`
...
- `subtree_rollback_requested`: rollback requested restoration of a previous subtree hash.
- `subtree_rolled_back`: rollback restored the previous subtree at the recorded tick boundary.
- `subtree_rollback_rejected`: rollback was rejected and the current tree remained active.
