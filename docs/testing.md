...
obsolete capability calls and, for context change, paired mallet-trajectory
separation towards the no-longer authorised target, completes the episode with current-context
...
trajectories rather than attributing it to the stale call. Owner pre-emption is
an authority/capability-call test because the recovery branch can independently
select the same target values.
