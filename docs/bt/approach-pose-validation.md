...
It does not send a walking command.
...
The public types are declared in `bt/approach_pose_validator.hpp`:
...
The host retains no ownership. The validator must outlive every tick that can
call it. Passing `nullptr` unregisters it.
