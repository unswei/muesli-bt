...
- complete the optional `muesli-model-service` bridge against `MMSP v0.2`: the first low-level `ws://` request/response client exists, stateless `cap.call` world-model requests can use it, the first `describe` compatibility gate checks required public capability ids, stateless calls have request/response hashes plus a request-hash keyed replay cache, the first stateless output validation gate rejects missing, stale, unsafe, or policy-violating proposals before host reach, stateless calls have deterministic fault schedules for invalid output, unsafe output, stale result, timeout, unavailable, and delay injection, and the first VLA session adapter maps the existing `vla.submit` / `vla.poll` / `vla.cancel` lifecycle onto `MMSP v0.2` `start` / `step` / `cancel` / `close`; runtime hardening still needs deeper descriptor mode/schema checks, explicit `status` operation coverage if required by a backend, redaction, richer replay reports, host action dispatch validation, session replay parity, session fault schedules, and evidence outputs
