...
- complete the optional `muesli-model-service` bridge against `MMSP v0.2`: the first low-level `ws://` request/response client exists, stateless `cap.call` world-model requests can use it, the first `describe` compatibility gate checks required public capability ids, stateless calls have request/response hashes plus a request-hash keyed replay cache, and the first stateless output validation gate rejects missing, stale, unsafe, or policy-violating proposals before host reach; runtime hardening still needs deeper descriptor mode/schema checks, VLA `start` / `step` / `cancel` / `status` / `close`, redaction, richer replay reports, host action dispatch validation, and evidence outputs
