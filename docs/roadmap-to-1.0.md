...
- complete the optional `muesli-model-service` bridge against `MMSP v0.2`: the first low-level `ws://` request/response client exists, stateless `cap.call` world-model requests can use it, the first `describe` compatibility gate checks required public capability ids, and stateless calls have request/response hashes plus a request-hash keyed replay cache; runtime hardening still needs deeper descriptor mode/schema checks, VLA `start` / `step` / `cancel` / `status` / `close`, redaction, richer replay reports, and evidence outputs
