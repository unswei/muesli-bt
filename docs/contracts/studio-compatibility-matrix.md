...
| `main` branch (rolling) | `1.0.0` until changed by contract versioning | `mbt.evt.v1` until schema bump | parser for active schema required | push/PR CI plus Linux ROS-backed checks, rosbag-backed `L2` replay on Ubuntu 22.04 + Humble, and canonical ROS artefact verification through `events.jsonl` | best-effort (non-stable) | rolling branch after the `v0.3.1` packaging baseline; direct canonical ROS-backed replay/log parity is now landed on `main`, but tag-level support posture still depends on the next release |
