...
public:
    ...
    planner_vector rollout_action(const planner_vector& state, planner_rng& rng) const override {
        ...
        const double throttle = clamp_double(0.45 + rng.normal(0.0, 0.20), 0.0, 1.0);
    ...
    bool validate_state(const planner_vector& state) const override {
        return state.size() >= 6 && vector_all_finite(state);
    }
...
planner_service::planner_service() {
    ...
    register_model("racecar-kinematic-v1", std::make_shared<racecar_kinematic_model>());
}
