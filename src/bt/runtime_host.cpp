...
runtime_host::runtime_host() : scheduler_(0), logs_(4096) {}
...
std::string runtime_host::dump_logs() const {
    ...
    for (const log_record& rec : logs_.snapshot()) {
        out << rec.sequence << " level=" << log_level_name(rec.level) << " tick=" << rec.tick_index
            << " node=" << rec.node << " category=" << rec.category << " msg=" << rec.message << '\n';
    }
    return out.str();
}
