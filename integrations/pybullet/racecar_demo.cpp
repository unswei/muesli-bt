...
void validate_state_schema(const racecar_state& state) {
    if (state.state_schema != "racecar_state.v1") {
        throw std::runtime_error("pybullet.observe: state_schema must be racecar_state.v1");
    }
    if (state.goal.size() < 2) {
        throw std::runtime_error("pybullet.observe: goal must be [gx gy]");
    }
    if (state.state_vec.empty()) {
        throw std::runtime_error("pybullet.observe: state_vec must not be empty");
    }
    for (double v : state.state_vec) {
        if (!std::isfinite(v)) {
            throw std::runtime_error("pybullet.observe: state_vec contains non-finite value");
        }
    }
    for (double v : state.rays) {
        if (!std::isfinite(v)) {
            throw std::runtime_error("pybullet.observe: rays contains non-finite value");
        }
    }
    (void)finite_or_throw(state.x, "pybullet.observe x");
    (void)finite_or_throw(state.y, "pybullet.observe y");
    (void)finite_or_throw(state.yaw, "pybullet.observe yaw");
    (void)finite_or_throw(state.speed, "pybullet.observe speed");
}
...
void write_state_to_blackboard(instance& inst,
    ...
    inst.bb.put(options.action_key, bb_value{std::monostate{}}, tick_index, now, 0, "env.run-loop");
    if (!options.planner_meta_key.empty()) {
        inst.bb.put(options.planner_meta_key, bb_value{std::monostate{}}, tick_index, now, 0, "env.run-loop");
    }
    inst.bb.put(options.state_key, bb_value{state.state_vec}, tick_index, now, 0, "env.run-loop");
    inst.bb.put("state_schema", bb_value{state.state_schema}, tick_index, now, 0, "env.run-loop");
    inst.bb.put("x", bb_value{state.x}, tick_index, now, 0, "env.run-loop");
    inst.bb.put("y", bb_value{state.y}, tick_index, now, 0, "env.run-loop");
    inst.bb.put("yaw", bb_value{state.yaw}, tick_index, now, 0, "env.run-loop");
    inst.bb.put("speed", bb_value{state.speed}, tick_index, now, 0, "env.run-loop");
    inst.bb.put("rays", bb_value{state.rays}, tick_index, now, 0, "env.run-loop");
    inst.bb.put("goal", bb_value{state.goal}, tick_index, now, 0, "env.run-loop");
...
racecar_state racecar_get_state() {
    ...
    if (!adapter) {
        throw std::runtime_error("pybullet.observe: racecar sim adapter is not installed");
    }
...
void racecar_apply_action(double steering, double throttle) {
    const std::shared_ptr<racecar_sim_adapter> adapter = racecar_sim_adapter_ptr();
    if (!adapter) {
        throw std::runtime_error("pybullet.act: racecar sim adapter is not installed");
    }
...
void racecar_step(std::int64_t steps) {
    const std::shared_ptr<racecar_sim_adapter> adapter = racecar_sim_adapter_ptr();
    if (!adapter) {
        throw std::runtime_error("pybullet.step: racecar sim adapter is not installed");
    }
    if (steps < 1) {
        throw std::runtime_error("pybullet.step: steps must be >= 1");
    }
    adapter->step(steps);
}
...
void racecar_reset() {
    const std::shared_ptr<racecar_sim_adapter> adapter = racecar_sim_adapter_ptr();
    if (!adapter) {
        throw std::runtime_error("pybullet.reset: racecar sim adapter is not installed");
    }
    adapter->reset();
}
...
void racecar_debug_draw() {
    const std::shared_ptr<racecar_sim_adapter> adapter = racecar_sim_adapter_ptr();
    if (!adapter) {
        throw std::runtime_error("pybullet.debug-draw: racecar sim adapter is not installed");
    }
    adapter->debug_draw();
}
...
racecar_loop_result run_racecar_loop(runtime_host& host, std::int64_t instance_handle, const racecar_loop_options& options) {
    if (options.tick_hz <= 0.0 || !std::isfinite(options.tick_hz)) {
        throw std::runtime_error("env.run-loop: :tick_hz must be finite and > 0");
    }
    if (options.max_ticks <= 0) {
        throw std::runtime_error("env.run-loop: :max_ticks must be > 0");
    }
    if (options.steps_per_tick <= 0) {
        throw std::runtime_error("env.run-loop: :steps_per_tick must be > 0");
    }
    if (options.state_key.empty() || options.action_key.empty()) {
        throw std::runtime_error("env.run-loop: :state_key and :action_key must be non-empty");
    }

    const std::shared_ptr<racecar_sim_adapter> adapter = racecar_sim_adapter_ptr();
    if (!adapter) {
        throw std::runtime_error("env.run-loop: racecar sim adapter is not installed");
    }

    instance* inst = host.find_instance(instance_handle);
    if (!inst) {
        throw std::runtime_error("env.run-loop: unknown bt instance");
    }
