...
| --- | --- | --- | --- |
| `sim.get-state`, `sim.apply-action`, `sim.step`, `sim.reset`, `sim.debug-draw`, `sim.run-loop` (renamed to `env.pybullet.*`) | `src/builtins.cpp` (before extraction), now `ext/pybullet_racecar/extension.cpp` | `tests/test_main.cpp`, racecar docs/example support | move |
