...
- `flagship_wheeled/`: reusable wheeled BT, shared contract helpers, thresholds, and run-normalisation tooling.
...
- `webots_epuck_goal/`: goal seeking in a cluttered arena with lidar-lite observations.
  Includes `lisp/educational_goal_bt.lisp` for a single-file teaching version and `lisp/flagship_entry.lisp` for a reusable wrapper version.
...
- `repl_scripts/ros2-flagship-goal.lisp`: ROS2 wrapper that derives the wheeled demo contract from odometry plus fixed scenario geometry.
